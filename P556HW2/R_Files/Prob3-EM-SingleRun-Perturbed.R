# This is the perturbed version of the EM. A small sized diagonal matrix 
# is added to the covariance matrix to address the singularity problem
library(mvtnorm)
...
epsMat <- diag(x = perturbedEps, nrow = d)
...
while (change > tol && itr < maxItr) {
    ...
    # Expectation step
    for (i in 1:k) 
        wt[, i] <- dmvnorm(x = obs, mean = mu[, i], sigma = covMat[,, i] + epsMat) * prior[i]
